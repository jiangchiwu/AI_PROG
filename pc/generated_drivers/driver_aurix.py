...
class DriverAURIX TC系列驱动(ChipDriverBase):
    """AURIX TC系列驱动系列芯片驱动"""
    
    driver_name = "AURIX"
    driver_version = "1.0.0"
    supported_cores = ['TriCore']
    supported_debug_interfaces = ['JTAG']
    supported_vendors = ["Infineon"]
    supported_families = ['TC*']
    ...
    def init(self, config: dict) -> bool:
        ...
        # 初始化调试接口
        debug_type = config.get('debug_interface', 'JTAG')
    ...
    def detect(self) -> dict:
        """检测芯片"""
        # 读取芯片ID
        die_id = self.debug_interface.read_memory(0xF0080000, 32)
